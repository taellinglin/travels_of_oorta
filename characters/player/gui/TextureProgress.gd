...
func update_value(new_value: float) -> void:
	$TweenColor.interpolate_property(self, 'modulate', modulate, Color(255, 255, 255), 1, Tween.TRANS_LINEAR, Tween.EASE_OUT_IN)
	$TweenColor.start()
	
	$TweenProgress.interpolate_property(self, 'value', value, new_value, 0, Tween.TRANS_ELASTIC, Tween.EASE_OUT)
	$TweenProgress.start()
